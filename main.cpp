...
namespace
{
    ...
    ClothSystem* cloth_system = NULL;
    int cur_system_idx = 2;
  ...
  void initSystem(int argc, char * argv[])
  {
    ...
    cloth_system = static_cast<ClothSystem*>(system);
    // seed the random number generator with the current time
    srand( time( NULL ) );

    if (argc != 3) {
        cout << "./a3 <INTEGRATOR> <STEP_SIZE>" << endl;
        exit(0);
    }

    // The first argument selects the solver
    string arg_solver(argv[1]);
    if (arg_solver == "e") {
        timeStepper = new ForwardEuler();
    } else if (arg_solver == "t") {
        timeStepper = new Trapzoidal();
    } else if (arg_solver == "r") {
        timeStepper = new RK4();
    } else {
        cerr << "Invalid solver argument, expecting either e, t, or r, but now is " << arg_solver << endl;
        exit(0);
    }

    // The second argument is the stepsize
    string arg_h(argv[2]);
    try {
        h = stof(arg_h);
    }
    catch (const std::invalid_argument& ia) {
        std::cerr << "Invalid step_size argument, must be a float number, but now is " << arg_h << endl;
        exit(0);
    }

    cout << "Key Events:" << endl;
    cout << "ESCAPE	- quit the application" << endl;
    cout << "a - toggle cloth parameters" << endl;
    cout << "c - toggle cloth shaders" << endl;
    cout << "d - toggle wind" << endl;
    cout << "r - reset current system" << endl;
    cout << "s - toggle swing" << endl;
	cout << "t - toggle between systems" << endl;
  ...
  void drawSystem()
  {
    ...
    void setSystem(int cur_system_idx) {
        delete(system);
        if (cur_system_idx == 0) {
            system = new SimpleSystem();
        } else if (cur_system_idx == 1) {
            system = new PendulumSystem(5);
        } else if (cur_system_idx == 2) {
            system = new ClothSystem(20, 20);
            cloth_system = static_cast<ClothSystem*>(system);
        }
    }
    ...
    void keyboardFunc( unsigned char key, int x, int y )
    {
        ...
        case 'a':
            if (cloth_system) {
                cloth_system->toggleCloth();
            }
            break;
        case 'c':
            if (cloth_system) {
                cloth_system->toggleShading();
            }
            break;
        case 'd':
            if (cloth_system) {
                cloth_system->toggleWind();
            }
            break;
        case 's':
            if (cloth_system) {
                cloth_system->toggleSwing();
            }
            break;
        case 't':
            cur_system_idx = (cur_system_idx + 1) % 3;
            setSystem(cur_system_idx);
            break;
        case 'r':
...
int main( int argc, char* argv[] )
{
    ...
    camera.SetDimensions( 1500, 1000 );
